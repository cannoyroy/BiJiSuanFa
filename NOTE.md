...
原理是hash，建立一个结构体数组，下标表示地址，且不需要头结点。
...
以深度为前进的关键词，不碰到死胡同不回头；一种枚举所有完整路径以遍历所有情况的搜索方法。

剪枝：通过题目条件的限制来节省DFS计算量

### 8.2 广度优先搜索（BFS）

总是先依次访问从该岔道口能直接到达的所有结点，然后再按这些结点被访问的顺序去一次访问它们能直接到达的所有结点
...
数据结构中，有结点（node），树根抽象为根结点（root），树叶概括为叶子结点（leaf），边（edge）

1. 树可以没有结点，称为空树（empty tree）
2. 树的层次（layer）从根节点算起，即根节点为第一层
3. 把结点的子树棵树成为结点的度（degree），而树中结点最大的度成为树的度（也称为树的宽度）
4. 由于一条边连接两个结点，且树中不存在环，因此n个结点的树，边树是n-1。且满足连通、边树等于顶点树减1的结构一定是树
5. 叶子结点被定义为度为0的结点，因此树中只有一个根结点时，根结点就是叶子结点
6. 结点的深度指从根结点（depth=1）开始自顶向下累加至该结点时的深度指；结点的高度是指从最底层叶子（height=1）结点开始向上累加的高度值。树的深度/高度为所有深度/高度的最大值
7. 多棵树组合在一起称为森林
